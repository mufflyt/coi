...
GOBA <- GOBA_unique[,c(4,13,15,18)]
GOBA_Compare <- sqldf('select GOBA.*, StudyGroup.PPI from GOBA left outer join StudyGroup on GOBA.NPI_Match = StudyGroup.NPI')
...
GOBAx <- sqldf('select GOBAx.*, Prescriber_Name.npi from GOBAx left outer join Prescriber_Name on GOBAx.NPI_Match = Prescriber_Name.npi')
